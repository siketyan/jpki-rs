    ...
    Ctx: Copy,
{
    ...
    pub fn read_surface(&self, ctx: Ctx, pin: Pin) -> Result<Vec<u8>, ApduError> {
        match pin {
    ...
    fn verify_pin_a(&self, ctx: Ctx, pin: Vec<u8>) -> Result<(), ApduError> {
        self.card.verify_pin(ctx, EF_PIN_A, pin)
    }

    fn verify_pin_b(&self, ctx: Ctx, pin: Vec<u8>) -> Result<(), ApduError> {
